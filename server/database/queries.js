...
module.exports.SUBMIT13 = (ObjetoRecolhido, DisturbioComportamento, EncontradoCapacete, EncontradoCinto, ParaBrisasAvariado, CaminhandoCena, PainelAvariado, VolanteTorcido) => {
    return `INSERT INTO avaliacao_cinematica (Distubio_Comportamento, Encontrado_de_Capacete, Encontrado_de_Cinto, Para-Brisas_Avariado, Caminhando_na_Cena, Painel_Avariado, Volante_Torcido, Objetos_Recolhidos) 
...
module.exports.SUBMIT8 = (
    ...
    return `
        INSERT INTO Problemas_Encontrados_Suspeitos (Psiquiatrico, Parto_Emergencial, Gestante, Hemorragia_Excessiva, Aerio, Clinico, Emergencial, Pos-trauma, SAMU, Sem_Remocao, Transporte_Outros, Outros, Diabetes, Respiratorio) VALUES ("${psico}", "${outrosProblemas}", "${partoEmergencial}", "${problemaGestante}", "${hemorragiaExcessiva}", "${aereo}", "${clinico}", "${emergencial}", "${posTrauma}", "${samu}", "${semRemocao}", "${outrosProblemaTransporte}", "${diabetes}", "${resp}");
    `;
};
...
module.exports.SUBMIT10 = (
    ...
    return `
        INSERT INTO procedimentos_efetuados (Aspiracao, Avaliacao_Inicial, Avaliacao_Digerida, Avaliacao_Continuada, Chave_de_Rautek, Canula_de_Guedel, Desobstrucao_de_V.A, Emprego de D.E.A, Gerenciamento_de_Riscos, Limpeza_de_Ferimentos, Curativos, Compressivo, Encravamento, Ocular, Queimadura, Simples, 3 Pontas, Imobilizacoes, Membro_Inferior_Direito, Membro_Inferior_Esquerdo, Membro_Superior_Direito, Membro_Superior_Esquerdo, Quadril, Cervical, Maca_sobre_Rodas, Maca_Rigida, Ponte, Retirado_Capacete, R.C.P, Rolamento_90, Rolamento_180, Tomada_Decisao, Tratado_Choque, Uso_de_Canula, Uso_Colar, Tamanho, USO_KED, USO_TTF, Ventilacao_Suporte, Oxigenoterapia, Oxigenoterapia_LPM, Reanimador, Reanimador_LPM, Celesc, Civil, Militar, PRE, PRF, Defesa_Civil, IGP_ou_PC, SAMU_USA, SAMU_USB, SAMU_Outros, CIT, Outro) 
    VALUER("${Aspiracao}", "${AvalInicial}", "${AvalDirigida}", "${AvalContinuada}", "${ChaveRautek}", "${CanulaGuedel}", "${DesobstrucaoVA}", "${EmpregoDEA}", "${GerenciamentoRiscos}", "${LimpezaFerimento}", "${Curativos}", "${Compressivo}", "${Encravamento}", "${Ocular}", "${Queimadura}", "${Simples}", "${TresPontas}", "${Imobilizacoes}", "${MacaRodas}", "${MacaRigida}", "${Ponte}", "${RetiradoCapacete}", "${Rcp}", "${Rolamento90}", "${Rolamento180}", "${TomadaDecisao}", "${TratadoChoque}", "${UsoCanula}", "${UsoColar}", "${UsoKED}", "${UsoTTF}", "${VentSuporte}", "${Oxigenoterapia}", "${Reanimador}", "${Meios}", "${Auxiliares}", "${OutrosProcedimentos}");`;
};
